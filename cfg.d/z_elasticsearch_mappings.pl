...
$c->{es_type_defintitions} = {
  boolean => {type => "boolean"},
  text => { type => "text", index_options => "offsets"},
  longtext => { type => "text", index_options => "offsets"},
  id => { type => "keyword"},
  date => { type => "date"},
  time => { type => "date"},
  timestamp => { type => "date"},
...
$c->{es_mapping_type_name} = sub
{
  ...
  {
     $properties->{$sub_field->name} = $c->{"es_type_defintitions"}->{$sub_field->type};
  }
  my $compound_mapping = {
    $field->name => {
      type => "nested",
...
$c->{es_mapping_type_compound} = sub
{
  my ($field) = @_;
